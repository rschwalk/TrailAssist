...
int TrailAssist::Trail::calculate_daily_distance() const
{
  ...
  //
  const auto min_possible_distance_at = std::ranges::max_element(m_hut_distances.begin(), m_hut_distances.end());
  int min_possible_distance = *min_possible_distance_at;

  // NOLINTNEXTLINE(boost-use-ranges)
  int max_possible_distance = std::accumulate(m_hut_distances.begin(), m_hut_distances.end(), 0);
...
bool TrailAssist::Trail::check_hike_possibility(const int max_day_distance) const
{
  ...
  for (const auto &hut_distance : m_hut_distances) {
    ...
    if (day_count > m_days) { return false; }
  }
...
void TrailAssist::Trail::print_result(const int max_day_distance) const
{
  ...
  if (day_count < m_days) {
    auto last_day_distance = m_hut_distances.back();
  ...
  try {
    ...
    while (std::getline(file_content, line)) {
      ...
      case 0:
        ...
        break;
      case 1:
        trail.m_days = std::stoi(line);
        break;
      default:
        trail.m_hut_distances.push_back(std::stoi(line));
        break;
      }
      count++;
    }
